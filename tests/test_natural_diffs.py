...
def _get_change_summary(edits):
    ...
    for e in edits:
        if e.operation == EditOperationType.DELETION:
            summary.append(f"[-'{e.target_text_to_change_or_anchor}']")
        elif e.operation == EditOperationType.INSERTION:
            summary.append(f"[+'{e.proposed_new_text}']")
        elif e.operation == EditOperationType.MODIFICATION:
            summary.append(f"['{e.target_text_to_change_or_anchor}'->'{e.proposed_new_text}']")
    return " ".join(summary)
...
def test_legal_term_replacement_tenant_lessee():
    ...
    target_edits = [e for e in edits if "enant" in e.target_text_to_change_or_anchor or "essee" in (e.proposed_new_text or "")]
    
    assert len(target_edits) == 1, f"Fragmented diffs found: {_get_change_summary(edits)}"
    assert target_edits[0].target_text_to_change_or_anchor == "Tenant"
    assert target_edits[0].proposed_new_text == "Lessee"
...
def test_corporate_name_change():
    ...
    solutions_edits = [e for e in edits if "Solutions" in e.target_text_to_change_or_anchor]
...
def test_date_change_fragmentation():
    ...
    oct_edits = [e for e in edits if "October" in e.target_text_to_change_or_anchor]
...
def test_indemnification_clause_rewrite():
    ...
    indemnify_edit = [e for e in edits if e.target_text_to_change_or_anchor == "indemnify"]
...
def test_multi_word_phrase_change():
    ...
    phrase_edits = [e for e in edits if "quick" in e.target_text_to_change_or_anchor]
    ...
    if phrase_edits[0].operation == EditOperationType.MODIFICATION:
        assert "brown" in phrase_edits[0].target_text_to_change_or_anchor
        assert "slow" in phrase_edits[0].proposed_new_text
        assert "red" in phrase_edits[0].proposed_new_text