...
"""CC-11 â€” Adeu opens every WordprocessingML flavour, and preserves which one it was.
...
was a dual-engine parity break as well as a product gap â€” templates are the
...
def test_save_preserves_the_flavour(tmp_path: Path, content_type: str, extension: str, expect_template: bool) -> None:
    ...
    convert the user's template into a document â€” fidelity loss inflicted by a
    read.
    """
    data = _build(tmp_path, content_type, extension).read_bytes()
