...
def test_tc1_sequential_chaining_works():
    """
    TC1 [report F1], extended 2026-07-17: sequential chaining is a first-class
    batch semantic. Each edit is validated and applied against the document
    state produced by the edits before it, so a second modify may target the
    first modify's output.
    """
    doc = Document()
    ...
    res_wet = engine.process_batch(batch())
    assert res_wet["edits_applied"] == 2
    assert res_wet["edits_skipped"] == 0
    assert all(r["status"] == "applied" for r in res_wet["edits"])
...
def test_tc2_write_applies_active_insertion_edit():
    """
    TC2: a single (strict) modification whose target lies entirely inside
    another author's pending insertion is applied: track_delete_run splits the
    enclosing <w:ins> and nests the change. (Partial overlaps and
    match_mode="all" fan-outs are still refused — see
    TestQaReportV2EngineSafety.)
    """
    doc = Document()
    ...
    res_wet = engine.process_batch([ModifyText(target_text="five (5)", new_text="seven (7)")])
    assert res_wet["edits_applied"] == 1
    assert res_wet["edits_skipped"] == 0
    assert res_wet["edits"][0]["status"] == "applied"
...
def test_tc5_w16du_namespace_untouched_parts():
    ...
    engine = RedlineEngine(stream)
    engine.process_batch([ModifyText(target_text="untouched body", new_text="changed body")])
...
def test_tc8_error_message_mislabeled_index():
    ...
    Intended behavior: when a subsequent edit fails, the transactional
    rejection's error message must be labeled with the 1-based index of THAT
    edit (not of the first edit in the batch).
    """
    import pytest
