...
- **Whole-Document Revision**: Rewrites a document from revised clean text (`adeu_apply_text_revision`), diffing it against the clean view and materializing the delta as tracked changes behind a post-apply verification gate.
- **Tracked-Changes Ledger**: Inspects active revisions, insertions, deletions, and comment threads with stable IDs across multi-turn workflows (`mode="changes"`).
...
<CodeGroup>
  ```python pip pip install -U langchain-adeu ``` ```python uv uv add
  langchain-adeu ```
</CodeGroup>
...
Instantiate the `AdeuToolkit` to retrieve all six document processing tools:
...
# Inspect active tracked changes and comment threads with stable IDs
changes_ledger = read_tool.invoke({
    "reasoning": "Collecting active tracked-change IDs before resolving revisions.",
...
model_generated_tool_call = {
    "args": {
        "reasoning": "Standardizing the governing-law jurisdiction to Delaware.",
        "file_path": "path/to/agreement.docx",
