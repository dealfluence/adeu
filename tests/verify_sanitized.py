...
def assert_empty(elements, message):
    if elements:
        ...
        for el in elements[:3]:
            print(f"   Found: {etree.tostring(el).decode('utf-8').strip()}")
        if len(elements) > 3:
...
def check_global_scrub(zf: zipfile.ZipFile):
    ...
    if core_tree:
        ...
        if creators and creators[0].strip():
            print(f"❌ FAIL: dc:creator not scrubbed. Found: {creators[0]}")
            sys.exit(1)

        modifiers = core_tree.xpath("//cp:lastModifiedBy/text()", namespaces=NAMESPACES)
        if modifiers and modifiers[0].strip():
            print(f"❌ FAIL: cp:lastModifiedBy not scrubbed. Found: {modifiers[0]}")
            sys.exit(1)
        ...
        for tag in ["dcterms:created", "dcterms:modified"]:
            ...
            if times and times[0] != "1970-01-01T00:00:00Z":
                print(f"❌ FAIL: {tag} not normalized. Found: {times[0]}")
                sys.exit(1)
    ...
    if app_tree:
        for tag in ["app:TotalTime", "app:Template", "app:Company", "app:Manager"]:
            ...
            if vals and vals[0].strip() and vals[0].strip() != "0":
                print(f"❌ FAIL: {tag} not scrubbed. Found: {vals[0]}")
                sys.exit(1)

    # 4. Check for Custom XML parts
    custom_xml_files = [f for f in zf.namelist() if f.startswith("customXml/")]
    if custom_xml_files:
        print(f"❌ FAIL: Custom XML parts were not stripped: {custom_xml_files}")
        sys.exit(1)
...
def check_full_scrub(zf: zipfile.ZipFile):
    ...
    if "word/comments.xml" in zf.namelist():
        print("❌ FAIL: word/comments.xml still exists in package.")
        sys.exit(1)
...
def check_keep_markup(zf: zipfile.ZipFile, expected_author: str):
    ...
    if doc_tree:
        ...
        for c in changes:
            ...
            if author and author != expected_author:
                print(f"❌ FAIL: Track change author '{author}' does not match expected '{expected_author}'")
                sys.exit(1)
            
            date = c.get(f"{{{NAMESPACES['w']}}}date")
            if date and date != "2025-01-01T00:00:00Z":
                print(f"❌ FAIL: Track change date '{date}' was not normalized")
                sys.exit(1)
    ...
    if comments_tree:
        ...
        for c in comments:
            author = c.get(f"{{{NAMESPACES['w']}}}author")
            if author and author != expected_author:
                print(f"❌ FAIL: Comment author '{author}' does not match expected '{expected_author}'")
                sys.exit(1)
