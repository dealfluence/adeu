...
*   **Nested Redline Strict Refusal**: Edits that target text strictly inside an active `<w:ins>` authored by a *different* user are explicitly rejected (`BatchValidationError`). Virtual `TextSpan` instances also explicitly track `comment_ids` to trigger safety rollbacks if an edit (such as an `all`-mode replacement) overlaps a `COMMENT_ONLY` annotation from a foreign author.
