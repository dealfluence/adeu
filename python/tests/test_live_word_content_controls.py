...
"""CC-6: what real Word does to content controls — measured, not assumed.

Task: specs/content-controls/TASKS.md CC-6. These tests are the deliverable of
that task: every `[COM-PENDING]` claim in spec-gates.md and spec-set-field.md is
either confirmed or amended here, and pinned so a future Word build that changes
its mind fails the build instead of quietly invalidating the spec.

Findings are written up in specs/content-controls/PROGRESS.md; the short version
of the ones that contradicted the frozen spec:
