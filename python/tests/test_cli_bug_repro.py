...
def test_read_docx_negative_page_boundary(tmp_path):
    ...
    # Calling with page=-1 must raise a ToolError
    with pytest.raises(ToolError) as exc_info:
        asyncio.run(read_docx(reasoning="test", file_path=str(doc_path), ctx=ctx, page=-1))

    assert "Invalid page parameter: '-1'" in str(exc_info.value) or "Page -1 out of range" in str(exc_info.value)
    ...
    assert "Invalid page parameter: '0'" in str(exc_info_zero.value) or "Page 0 out of range" in str(
        exc_info_zero.value
    )
...
def test_diff_json_reapplicability_after_heading_deletion(tmp_path, capsys):
    """
    Verifies that `adeu diff --json` output can be sequentially applied by `adeu apply`
    when deleting heading paragraphs alongside inline/paragraph edits.

    Regression test for bug where `adeu diff --json` emitted a bare '\\n\\n' newline target,
    which fails validation in `adeu apply` due to virtual projection chrome dropping.
    """
    import docx

    # 1. Create original document with a heading and two paragraphs
    doc_path = tmp_path / "orig.docx"
    doc = docx.Document()
    ...
    # 2. Create modified text file without heading and with paragraph edit
    txt_path = tmp_path / "mod.txt"
    txt_path.write_text("Paragraph A\nParagraph B modified\n", encoding="utf-8")

    diff_json_path = tmp_path / "diff.json"
    applied_doc_path = tmp_path / "applied.docx"

    # 3. Generate diff JSON
    code_diff, stdout_diff, stderr_diff = run_cli(
        ["diff", str(doc_path), str(txt_path), "--json", "-o", str(diff_json_path)],
        capsys,
    )
    assert code_diff == 0, f"adeu diff --json failed with code {code_diff}.\nSTDERR:\n{stderr_diff}"
    assert diff_json_path.exists(), "diff.json was not created"

    # 4. Apply generated diff JSON back to original document
    code_apply, stdout_apply, stderr_apply = run_cli(
        ["apply", str(doc_path), str(diff_json_path), "-o", str(applied_doc_path)],
        capsys,
    )

    # 5. Assert diff closure property: adeu apply MUST succeed with code 0
    assert code_apply == 0, (
        f"adeu apply failed to apply diff.json generated by adeu diff (code {code_apply}).\n"
        f"STDERR:\n{stderr_apply}\nSTDOUT:\n{stdout_apply}"
    )
    assert applied_doc_path.exists(), "applied.docx was not created"
